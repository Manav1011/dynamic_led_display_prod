...
async def send_messages(websocket,data=None):
    try:        
        if data is not None:
            await websocket.send(json.dumps({'client':'producer','action':'stream','data':data,'mode':'serial_rs232'}))
        else:
            await websocket.send(json.dumps({'client':'producer','action':'connection','mode':'serial_rs232'}))
...
if __name__ == "__main__":
    async def connect_to_websocket():
        async with websockets.connect(f"wss://192.168.29.18:8000/serial_communication_rs232/") as websocket:
